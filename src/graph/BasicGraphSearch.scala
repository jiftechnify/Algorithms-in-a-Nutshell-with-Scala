...
object BasicGraphSearch {
  def dfsAll[A](graph: Graph[A], start: A): mutable.Map[A, Option[A]] = {
  ...
  /**
    ...
    */
  def dfs[A](graph: Graph[A], start: A, goal: A): List[A] = {
  ...
  /**
    ...
    * @return 起点から終点への経路
    */
  def dfs2[A](graph: Graph[A], start: A, goal: A): List[A] = {
  ...
  /**
    ...
    */
  def calcBfsDistance[A](graph: Graph[A], from: A): Map[A, Int] = {
